    ...
    <div class="grid sm:grid-cols-1 md:grid-cols-3 gap-1 p-2 w-auto rounded-lg ">
        ...
        <div class="mb-6 ring-1 ring-zinc-600 rounded-md p-2">
            ...
            <div class="relative z-0 w-full mb-5 group">
                <input type="date" wire:model.live="filtrodesde" class="block py-2.5 px-0 w-full text-xs md:text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"  />
                <label for="filtrodesde" class="peer-focus:font-medium absolute text-xs md:text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Hasta que fecha (Mostrará 7 días adelante a la fecha elegida.)</label>
            </div>
        </div>
    </div>

    @if ($is_consulta)
        <div class="grid sm:grid-cols-1 md:grid-cols-4 gap-2 bg-blue-100 p-2 w-auto rounded-lg ">
